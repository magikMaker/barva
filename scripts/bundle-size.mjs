...
function findMinifiedFile(pkgName) {
  try {
    // Special case for barva which is in the local project
    if (pkgName === 'barva') {
...
async function main() {
  ...
  try {
    ...
    // Display barva first
    const barvaInfo = getPackageInfo('barva');
    console.log(
      `${'barva'.padEnd(20)} ${String(barvaInfo.version).padEnd(10)} ${
        barvaInfo.minified.padStart(10)} ${barvaInfo.gzipped.padStart(10)} ${
        path.basename(barvaInfo.path || '')}`
    );
